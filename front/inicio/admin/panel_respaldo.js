...
function estadoColor(estado) {
 ...
  if (estado === 'averiado') return 'rojo';
    // Mapeo inverso si la BD usa 'Disponible', 'Pedido', 'Averiando'
    if (estado === 'Disponible') return 'verde';
    if (estado === 'Pedido') return 'amarillo';
    if (estado === 'Averiando') return 'rojo';
  return '';
}
function cargarRecursos() {
  fetch('/../../../back/conexion_re/listar_recursos.php')
    .then(res => res.json())
    .then(data => {
 ...
      } else {
        data.forEach(r => {
          // Asumiendo que listar_recursos devuelve claves Tipo y problema
          let tipo = r.Tipo || r.tipo; // Soporte para Tipo o tipo
          let estado = r.problema || 'disponible'; // Lee de la columna 'problema'

          ul.innerHTML += `<li>${tipo}</li>`;
...
document.addEventListener('DOMContentLoaded', function() {
 ...
      e.preventDefault();
      // LEER VALORES en minúsculas (nombres del HTML)
      const id_recurso = this.id_recurso.value; 
      const tipo = this.tipo.value; // Lee name="tipo"
      const estado = this.estado.value; // Lee name="estado"
